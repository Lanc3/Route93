    ...
    <div className={`relative ${className}`} ref={searchRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-96 overflow-y-auto">
          {query.length > 0 ? (
            <SearchCell search={query} limit={6} onProductClick={handleProductClick} />
          ) : (
            <div className="p-4">
              {/* Search History */}
              {searchHistory.length > 0 && (
                  ...
                  <div className="space-y-1">
                    {searchHistory.slice(0, 3).map((term, index) => (
                      <button
                        key={index}
                        onClick={() => handleHistoryClick(term)}
                        className="flex items-center space-x-2 w-full text-left px-2 py-1 rounded hover:bg-gray-100 transition-colors"
                      >
                ...
                <div className="flex flex-wrap gap-1">
                  {popularSearches.slice(0, 5).map((term, index) => (
                    <button
                      key={index}
                      onClick={() => handleHistoryClick(term)}
                      className="px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs hover:bg-purple-100 hover:text-purple-700 transition-colors"
                    >
                      {term}
                    </button>
                  ))}
